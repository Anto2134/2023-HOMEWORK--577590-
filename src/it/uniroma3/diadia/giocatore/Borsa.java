...
public class Borsa {
	...
	public List<Attrezzo> getContenutoOrdinatoPerPeso() {
		...
		Collections.sort(attrezziPerPeso, new ComparatoreAttrezziPesoNome());
	...
	public Map<Integer, Set<Attrezzo>> getContenutoRaggruppatoPerPeso() {
		...
		for(Attrezzo attrezzo : this.getAttrezzi()) {
			temp = mappa.get(attrezzo.getPeso());
			if(temp == null) 
				temp = new HashSet<Attrezzo>();
			temp.add(attrezzo);
			mappa.put(attrezzo.getPeso(), temp);
		}
		return mappa;
	}

	public SortedSet<Attrezzo> getSortedSetOrdinatoPerPeso() {
		SortedSet<Attrezzo> attrezziPerPeso = new TreeSet<>(new ComparatoreAttrezziPesoNome());
